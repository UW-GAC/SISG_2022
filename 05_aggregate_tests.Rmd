...
- Run the analysis in your project:
  ...
  - Specify the Inputs:
    ...
    - Variant group files: variants_by_gene.RData (has data for all chromosomes)
...
The output of this analysis will be 22 `_smmat_chr<CHR>.RData` files with the association test results for each chromosome as well as a `_smmat_manh.png` file with the Manhattan plot and a `_smmat_qq.png` file with the QQ plot. 

You can find the expected output of this analysis by looking at the existing task `9. 1KG SMMAT Association Test` in the Tasks menu of your Project. The output files are available in the Project, so you do not need to wait for your analysis to finish to look at the output.


### Exercise 5.3 (Data Studio)
